...
export class UserController {
  ...
  @Get(':id')
  async getUser(@Param('id') id: string): Promise<ResponseData<string>> {
    try {
      return new ResponseData<string>(this.userService.findOne(+id), HttpStatus.OK, HttpMessage.OK);
    } catch (error) {
      return new ResponseData<string>("Failed to get user", HttpStatus.INTERNAL_SERVER_ERROR, HttpMessage.INTERNAL_SERVER_ERROR);
    }

  }

  @Patch(':id')
  async updateUser(@Param('id') id: string, @Body() updateUserDto: UpdateUserDto): Promise<ResponseData<string>> {
    try {
      return new ResponseData<string>(this.userService.update(+id, updateUserDto), HttpStatus.OK, HttpMessage.OK);
    } catch (error) {
  ...
  async removeUser(@Param('id') id: string): Promise<ResponseData<string>> {
    try {
      return new ResponseData<string>(this.userService.remove(+id), HttpStatus.OK, HttpMessage.OK);
    } catch (error) {
