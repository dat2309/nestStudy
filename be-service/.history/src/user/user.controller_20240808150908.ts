...
export class UserController {
  ...
  async create(@Body() createUserDto: CreateUserDto): Promise<ResponseData<User | string>> {
    try {
      ...
      return new ResponseData<User>(result, HttpStatus.OK, HttpMessage.OK);
    } catch (error) {
      // Return a CustomError with the appropriate status and message
      if (error instanceof BadRequestException) {
        return new ResponseData<CustomError>(new CustomError(400, error.message), HttpStatus.OK, error.message);
      }
      return new ResponseData<CustomError>(new CustomError(500, "Failed to create user"), HttpStatus.OK, "Error occurred during creation");
    }
