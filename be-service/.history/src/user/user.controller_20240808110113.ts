...
export class UserController {
  ...
  @Get(':id')
  getUser(@Param('id') id: string) : Promise<ResponseData<User>>{
    try {
      return new ResponseData<User[]>(this.userService.findOne(+id), HttpStatus.OK, HttpMessage.OK);
    } catch (error) {
      return new ResponseData<User[]>(User(), HttpStatus.INTERNAL_SERVER_ERROR, HttpMessage.INTERNAL_SERVER_ERROR);
    }
