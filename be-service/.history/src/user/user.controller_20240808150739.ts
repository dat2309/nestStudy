...
export class UserController {
  ...
  async create(@Body() createUserDto: CreateUserDto): Promise<ResponseData<User | string>> {
    try {
      ...
      // Check if result contains a validation error format
      if (result && typeof result === 'object' && 'status' in result && result.status === 400) {
        return new ResponseData<string>(null, 400, result.message);
      }

      return new ResponseData<User>(result, HttpStatus.OK, HttpMessage.OK);
    } catch (error) {
      return new ResponseData<string>("Failed to create user", HttpStatus.OK, "Error occurred during creation");
    }
