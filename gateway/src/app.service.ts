...
export class AppService {
  ...
  async getZaloLoginUrl(): Promise<string> {
    try {
      ...
      throw new HttpException('Failed to fetch Zalo login URL', HttpStatus.INTERNAL_SERVER_ERROR);
    }
  }
  ...
  async checkSession(cookies: string[], req: any): Promise<string> {
    ...
    const validCookies = cookies.filter(cookie => !cookie.includes('EXPIRED'));
    ...
    try {
      let url = 'https://id.zalo.me/account/checksession?continue=https://chat.zalo.me';
      let lastResponse;
      ...
      while (true) {
        ...
        try {
          ...
          const response = await firstValueFrom(this.httpService.get(url, { headers, maxRedirects: 0 }));
            ...
            if (location) {
              if (location.includes('push')) {
                ...
                try {
                  ...
                  const finalResponse = await firstValueFrom(this.httpService.get(location, { headers: finalHeaders, maxRedirects: 0 }));
                }
                catch (error) {
                  if (error.response && error.response.status === 302) {
                    ...
                    return JSON.stringify({ cookies: responseCookies.join('; '), 'user-agent': req.headers['user-agent'], 'uuid': customUUID });
                  }
