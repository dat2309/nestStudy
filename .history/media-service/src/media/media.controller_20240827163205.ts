...
export class MediaController {
    ...
    async uploadFile(@UploadedFile() file: Express.Multer.File) : Promise<ResponseData<MediaResponse >> {
        if (!file) {
            throw new BadRequestException('No file uploaded');
          }
          try {
            console.log(file)
            return new ResponseData<UserResponse>(await this.userService.uploadAvatar(userId, file), HttpStatus.OK, 'Avatar uploaded successfully');
          } catch (error) {
